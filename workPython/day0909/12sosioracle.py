...
def sosiSelectTitle():
    #각자 조회갯수 출력 
    print('제목필드 like조회처리입니다  ')
    dtitle = input('제목조회>>> ')
    msg1 = f"select * from sosi where title like '%{dtitle}%' "
    # 에러msg2 = f"select * from sosi where title like '{%dtitle%}' "
    # SQL> select * from sosi where title like '%e%' ;
    print(msg1)
    print()
